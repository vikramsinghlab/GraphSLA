...
# Define the SGCConv model with additional layers
class SGCModel(nn.Module):
    def __init__(self, input_dim, num_layers=4):
        super(SGCModel, self).__init__()
        self.conv1 = SGConv(input_dim, 128, K=2)  
        self.conv2 = SGConv(128, 128, K=2)       
        self.conv3 = SGConv(128, 64, K=2)       
        self.conv4 = SGConv(64, 64, K=2)        
...
# Save the model
torch.save(model.state_dict(), 'sgc_SM_lncRNA_repurposing.pth')
